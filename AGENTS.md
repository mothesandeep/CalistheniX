...
# Project Structure
- `backend/` – Flask app: routes, SQLite access, sync endpoints
...
- `phases.md` – Phased build sequence (Steps 0–7) — check current phase before building ahead of it

# Environment Setup
```
cd backend
python -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt   # if present — otherwise install flask manually
python app.py                      # or the actual entrypoint filename
```
Frontend has no build step — open `frontend/index.html` directly or serve statically alongside the Flask app.

# Commands
There's no test suite yet. Until one exists:
- Run the backend and manually hit endpoints with `curl` or the browser
- Check the frontend by opening it in a browser and checking devtools console for JS errors
- When a test suite is added, document the exact file-scoped and full-suite commands here
...
- `phases.md` defines the build order (Steps 0–7) — don't jump ahead to a later phase's features without checking current progress first
