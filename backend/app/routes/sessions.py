...
@sessions_bp.route('/logs/batch', methods=['POST'])
def create_logs_batch():
    """Persist multiple log entries within a single atomic SQLite transaction.

    Accepts an array of log objects or an object with a 'logs' array:
      [ { exercise_id: 1, timestamp: '...', reps: 10, client_uuid: '...' }, ... ]
      or { "logs": [...] }
    Idempotently skips duplicate client_uuids and returns all saved items.
    """
    body = request.get_json(silent=True)
    if not body:
        return jsonify({'error': 'JSON body required'}), 400

    logs_list = body if isinstance(body, list) else body.get('logs', [])
    if not isinstance(logs_list, list):
        return jsonify({'error': 'logs array required'}), 400

    if not logs_list:
        return jsonify({'saved': 0, 'items': []}), 200
    ...
    with get_db() as conn:
        ...
        for item in logs_list:
            ...
            if not client_uuid or not timestamp:
                continue

            if not ex_id and item.get('exercise_name'):
                matched = conn.execute(
            ...
            phase_raw = (item.get('phase') or 'main').strip().lower().replace('-', '_')
            phase = 'warmup' if phase_raw in ('warmup', 'warm_up') else ('cooldown' if phase_raw in ('cooldown', 'cool_down') else 'main')
            ...
            try:
                ...
                row = conn.execute('SELECT * FROM logs WHERE client_uuid = ?', (client_uuid,)).fetchone()
                if row:
                    saved_items.append(dict(row))

        return jsonify({'saved': len(saved_items), 'items': saved_items}), 200
