...
let _screenWakeLockPromise = null;
...
async function acquireScreenWakeLock() {
  ...
  if (typeof navigator !== 'undefined' && 'wakeLock' in navigator && typeof navigator.wakeLock.request === 'function') {
    try {
      if ((!_screenWakeLock || _screenWakeLock.released) && !_screenWakeLockPromise) {
        _screenWakeLockPromise = navigator.wakeLock.request('screen');
        const lock = await _screenWakeLockPromise;
        _screenWakeLock = lock;
        _screenWakeLockPromise = null;
        if (_screenWakeLock && typeof _screenWakeLock.addEventListener === 'function') {
...
async function releaseScreenWakeLock() {
  const pending = _screenWakeLockPromise;
  _screenWakeLockPromise = null;
  if (pending) {
    try {
      const lock = await pending;
      if (lock && !lock.released && typeof lock.release === 'function') {
        await lock.release();
      }
    } catch {}
  }
