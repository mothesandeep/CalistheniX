...
(function () {
  ...
  function renderActivityHeatmap() {
    ...
    const monthsHtml = monthLabels.map((m) => {
      const offset = (m.weekIdx * 16.5);
      return `<span style="position:absolute; left:${offset}px;">${m.name}</span>`;
    }).join('');
  ...
  function renderEffortCard() {
    ...
    if (weeklyPoints.length >= 2) {
      ...
      sparklineSvg = `
        <div style="height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#64748b; font-size:12px; border:1px dashed rgba(255,255,255,0.06); border-radius:12px; padding:12px; text-align:center;">
          <span>No effort data in this period.</span>
          <span style="font-size:11px; color:#475569; margin-top:3px;">Log sets with RPE or RIR to visualize weekly effort trends.</span>
        </div>
      `;
    }
  ...
  function renderExerciseProgressCard() {
    ...
    if (pointsChronological.length >= 2) {
      ...
      chartSvg = `
        <div style="height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; color:#64748b; font-size:12.5px; border:1px dashed rgba(255,255,255,0.06); border-radius:12px; padding:16px;">
          <span>No workout logs recorded yet for ${exName}.</span>
          <span style="font-size:11px; color:#475569; margin-top:4px;">Complete a session with this exercise to track progress.</span>
        </div>
      `;
    }
  ...
  function renderRecentWorkoutsSection() {
    ...
    if (recentList.length > 0) {
      ...
      cardsHtml = `
        <div style="background:#14141c; border:1px dashed rgba(255,255,255,0.06); border-radius:18px; padding:24px; text-align:center;">
          <p style="color:#94a3b8; font-size:13.5px; margin:0 0 12px;">No completed workouts yet.</p>
