...
const workoutJsContent = fs.readFileSync(path.join(__dirname, "../js/views/workout-runner.js"), "utf8");
  ...
  const mockGlobals = {
    ...
    getActiveSession: () => (mockGlobals.state && mockGlobals.state.activeSession) || activeSessionState,
    saveActiveSession: (s) => { activeSessionState = s; if (mockGlobals.state) mockGlobals.state.activeSession = s; },
    openWorkoutView: () => {},
    showToast: () => {},
